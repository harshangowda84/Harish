 ...
 */
router.post("/write", auth_1.requireAuth, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        let registration;
        if (type === "passenger") {
            registration = await db_1.prisma.passengerRegistration.findUnique({
                where: { id: parseInt(registrationId) },
            });
        }
        else if (type === "student") {
            registration = await db_1.prisma.studentRegistration.findUnique({
                where: { id: parseInt(registrationId) },
            });
        }
        else {
        ...
        const uniquePassId = registration.uniquePassId || (0, rfid_1.generateUniquePassId)();
...
router.post("/read", async (req, res) => {
    try {
        ...
        if (simulate) {
            ...
            cardData = await (0, rfid_1.readFromRFIDCard)("COM5");
        }
        if (!cardData) {
        ...
        if (parsedData.id) {
            const passengerReg = await db_1.prisma.passengerRegistration.findFirst({
                where: { uniquePassId: parsedData.id },
            });
            const studentReg = await db_1.prisma.studentRegistration.findFirst({
                where: { uniquePassId: parsedData.id },
            });
            return res.json({
                success: true,
 ...
 */
router.get("/unique-pass-id/:passId", async (req, res) => {
    try {
